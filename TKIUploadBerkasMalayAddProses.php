...
$konektor = mysqli_connect("localhost","root","", "tki");
  ...
  $akta_malay = $_FILES['akta_malay']['name'];
  ...
  if($ektp_malay != "") {
    ...
    $akta = explode('.', $akta_malay); //memisamalayan nama file dengan ekstensi yang diupload
    $ekstensiakta = strtolower(end($akta));
    $file_tmpakta = $_FILES['akta_malay']['tmp_name'];   
    $angka_acak     = rand(1,999);
    $akta_baru = $angka_acak.'-'.$akta_malay;
    //surat nikah
    if($suratnikah_malay != "") {
    ...
    $skck_baru = $angka_acak.'-'.$skck_malay;
          ...
          if(in_array($ekstensiektp, $ekstensi_diperbolehkanektp) === true)  {     
                  ...
                  // jalankan query INSERT untuk menambah data ke database pastikan sesuai urutan (id tidak perlu karena dibikin otomatis)
                    $query = "INSERT INTO malaysia (sektor_malay, ektp_malay, kk_malay, akta_malay, suratnikah_malay, suratijin_malay, expaspor_malay, skck_malay, status_proses_malay) VALUES ('$sektor_malay', '$ektp_baru', '$kk_baru', '$akta_baru', '$suratnikah_baru', '$suratijin_baru', '$expaspor_baru', '$skck_baru', '$status_proses_malay')";
     ...
     $query = "INSERT INTO malaysia (sektor_malay, ektp_malay, kk_malay, akta_malay, suratnikah_malay, suratijin_malay, expaspor_malay, skck_malay, status_proses_malay) VALUES ('$sektor_malay', null, null, null, null, null, null, null, '$status_proses')";
                    $result = mysqli_query($konektor, $query);
                    // periska query apakah ada error
                    if(!$result){
                        die ("Query gagal dijalankan: ".mysqli_errno($konektor).
